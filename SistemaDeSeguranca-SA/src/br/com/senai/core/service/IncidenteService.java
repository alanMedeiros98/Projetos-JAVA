...
public class IncidenteService {
	...
	private void validar(Incidente incidente) {
		if (incidente != null) {
			boolean isDescricaoInvalida = incidente.getDescricaoCurta() == null
					|| incidente.getDescricaoCurta().isBlank() || incidente.getDescricaoCurta().length() < 2
			...
			boolean isGravidadeInvalida = incidente.getGravidade() <= 0 || incidente.getGravidade() > 1000;
	...
	public void excluirPor(int idDoIncidente) {
		if (idDoIncidente > 0) {
			boolean isRemocaoInvalida = daoIncidente.validarRemocao(idDoIncidente);
			if (isRemocaoInvalida) {
				throw new IllegalArgumentException(
						"Não é possivel remover um " + " incidente vinculado a uma ocorrência.");
