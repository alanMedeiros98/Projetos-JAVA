...
public class EnvolvidoService {
	...
	public void excluirPor(int idDoEnvolvido) {
		if (idDoEnvolvido > 0) {
			...
			this.daoEnvolvido.validarRemocao(idDoEnvolvido);
			boolean isRemocaoInvalida = daoEnvolvido.validarRemocao(idDoEnvolvido);
			if (isRemocaoInvalida) {
				throw new IllegalArgumentException(
						"Não é possivel remover um " + " envolvido vinculado a uma ocorrência.");
